        ...
        if username:
            ...
            if user_data:
                ...
                with st.sidebar:
                    ...
                    st.markdown(f"<h2 style='color:white'>👤 Username: {username}</h2>",unsafe_allow_html=True)
                    st.markdown(f"<h3 style='color:white'>📧 Email: {email}</h3>",unsafe_allow_html=True)
                    st.markdown(f"<h2 style='color:white'>🎂 Date of Birth: {dob}</h2>",unsafe_allow_html=True)
                    st.markdown(f"<h2 style='color:white'>🗓️ Age: {age}</h2>",unsafe_allow_html=True)
                    st.markdown("<hr style='border-top: 2px solid #bbb;'>", unsafe_allow_html=True)
            else:
